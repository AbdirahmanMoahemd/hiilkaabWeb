...
export const listProducts =
  ...
  export const listProductsPerPage = (keyword = '', pageNumber = '') => async (
    dispatch
  ) => {
    try {
      dispatch({ type: PRODUCT_LIST_REQUEST })
      ...
      const { data } = await axios.get(
        `/api/products/page?keyword=${keyword}&pageNumber=${pageNumber}`
      )
...
export const getProductsByCategory = (arg) => async (dispatch) => {
  try {
    dispatch({ type: PRODUCT_LIST_REQUEST });
...
export const getProductsBySubCategory = (arg) => async (dispatch) => {
  try {
    dispatch({ type: PRODUCT_LIST_REQUEST });
    const { data } = await axios.post("/api/products/search/sub", arg);
