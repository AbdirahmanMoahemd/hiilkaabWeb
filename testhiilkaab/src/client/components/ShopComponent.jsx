...
const ShopComponent = ({ products, max, min }) => {
  return (
   max !== 0 ? <div className="grid lg:grid-cols-2 xl:grid-cols-3 grid-cols-2 gap-6 mt-8">
      {/* <!-- single product --> */}
    ...
    <div className="grid lg:grid-cols-2 xl:grid-cols-3 grid-cols-2 gap-6 mt-8">
      {/* <!-- single product --> */}
      {products.filter((pro) => pro.price > min && pro.price < max).map((product) => (
        <div className="group rounded bg-white shadow overflow-hidden">
          {/* <!-- product image --> */}
          <Link to={`/product/${product.id}`}>
          <div className="flex items-center justify-center">
